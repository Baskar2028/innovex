...
const dbFile = path.join(dataDir, 'innovex.db');
...
function sqlRun(query, params = []) {
  return new Promise((resolve, reject) => {
    db.run(query, params, function onDone(error) {
      if (error) {
        reject(error);
        return;
      }
...
function validateRegistration(registration) {
  ...
  // Name validation
  if (!(typeof registration.name === 'string' && registration.name.trim().length > 0)) {
    return { valid: false, error: 'Name is required.' };
  }

  // Email validation
  if (!(typeof registration.email === 'string' && emailPattern.test(registration.email))) {
    return { valid: false, error: 'Please enter a valid email address.' };
  }

  // Contact validation
  if (!(typeof registration.contact === 'string' && contactPattern.test(registration.contact))) {
    return { valid: false, error: 'Please enter a valid 10-digit contact number.' };
  }

  // College validation
  if (!(typeof registration.college === 'string' && registration.college.trim().length > 0)) {
    return { valid: false, error: 'College name is required.' };
  }

  // Degree validation
  if (!allowedDegrees.includes(registration.degree)) {
    return { valid: false, error: 'Please select a valid degree.' };
  }

  // Branch validation
  if (!allowedBranches.includes(registration.branch)) {
    return { valid: false, error: 'Please select a valid branch.' };
  }
  ...
  if (!hasValidDetail) {
    return { valid: false, error: `Please enter your ${registration.degree === 'Others' ? 'degree' : 'Arts & Science degree'}.` };
  }

  // Team members validation
  if (!(Number.isInteger(registration.teamMembers) && registration.teamMembers >= 0 && registration.teamMembers <= 4)) {
    return { valid: false, error: 'Team members must be between 0 and 4.' };
  }

  // Members array validation
  if (!Array.isArray(registration.members)) {
    return { valid: false, error: 'Members data is invalid.' };
  }

  // Members count validation
  if (registration.members.length !== registration.teamMembers) {
    return { valid: false, error: 'Team members count does not match the members list.' };
  }
  ...
  if (registration.teamMembers > 0) {
    for (let i = 0; i < registration.members.length; i++) {
      ...
      if (!member) {
        return { valid: false, error: `Team member ${i + 1} data is invalid.` };
      }
      if (!(typeof member.name === 'string' && member.name.trim().length > 0)) {
        return { valid: false, error: `Team member ${i + 1}: Name is required.` };
      }
      if (!(typeof member.email === 'string' && emailPattern.test(member.email))) {
        return { valid: false, error: `Team member ${i + 1}: Please enter a valid email address.` };
      }
      if (!(typeof member.department === 'string' && member.department.trim().length > 0)) {
        return { valid: false, error: `Team member ${i + 1}: Department is required.` };
      }
      if (!(typeof member.contact === 'string' && contactPattern.test(member.contact))) {
        return { valid: false, error: `Team member ${i + 1}: Please enter a valid 10-digit contact number.` };
      }
      if (!(typeof member.college === 'string' && member.college.trim().length > 0)) {
        return { valid: false, error: `Team member ${i + 1}: College name is required.` };
      }
    }
  }
  ...
  if (!Array.isArray(registration.events)) {
    return { valid: false, error: 'Events data is invalid.' };
  }

  if (registration.events.length < 1 || registration.events.length > 2) {
    return { valid: false, error: 'Please select 1 or 2 events.' };
  }

  if (!registration.events.every((event) => allowedEvents.includes(event))) {
    return { valid: false, error: 'One or more selected events are invalid.' };
  }
...
function requireAdmin(req, res, next) {
  const keyFromQuery = typeof req.query.key === 'string' ? req.query.key : '';
  const keyFromBody = typeof req.body?.passcode === 'string' ? req.body.passcode : '';
  const keyFromHeader = typeof req.headers['x-admin-key'] === 'string' ? req.headers['x-admin-key'] : '';
  const providedKey = keyFromQuery || keyFromBody || keyFromHeader;
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Unable to save your registration. Please try again.' });
  }
});
...
app.delete('/api/admin/registrations/:id', requireAdmin, async (req, res) => {
  ...
  console.log(`DELETE /api/admin/registrations/:id called with id=${registrationId}`);

  if (!registrationId || isNaN(registrationId)) {
    console.log(`Invalid registration ID: ${registrationId}`);
    return res.status(400).json({ message: 'Invalid registration ID.' });
  }

  try {
    console.log(`Deleting registration with id=${registrationId}`);
    await sqlRun('DELETE FROM registrations WHERE id = ?', [Number(registrationId)]);
...
app.get('/api/admin/registrations', requireAdmin, async (req, res) => {
  try {
    const rows = await sqlAll('SELECT * FROM registrations ORDER BY id DESC');
    const registrations = rows.map(normalizeRegistration);
...
app.get('/api/export-csv', requireAdmin, async (req, res) => {
  try {
    const registrations = await sqlAll('SELECT * FROM registrations ORDER BY id DESC');
    ...
    const rows = registrations.map((item) => ({
      ...
      Degree: item.degree,
      'Degree Detail': item.degree_detail || 'N/A',
      Branch: item.branch,
      Year: item.year,
      'Team Members': item.team_members,
      'Team Member Details': parseJson(item.members_json, []).map((member, index) => `${index + 1}. ${member.name} | ${member.email} | ${member.department} | ${member.contact} | ${member.college}`).join('\n'),
      Events: parseJson(item.events_json, []).join(', '),
    ...
    return res.status(500).json({ message: 'Failed to export CSV file.' });
  }
});

app.get('/api/export-excel', requireAdmin, async (req, res) => {
  return res.redirect(302, `/api/export-csv?key=${encodeURIComponent(req.query.key || '')}`);
});
